...
public class Orienter 
{
	...
	**/
	...
	/**
	 * Test class
	 * @param args Forget this; it does nothing. 
	 */
	public static void main(String[] args)
	{
		for (int i = 0; i < 6; i++)
		{
			for (int j = 0; j < 6; j++)
			{
				double[] topVec = getDirection(UP_VECTOR, metadataTable[i][j]);
				double[] frontVec = getDirection(NORTH_VECTOR, metadataTable[i][j]);
				int[] topRounded = {(int) Math.round(topVec[0]), (int) Math.round(topVec[1]), (int) Math.round(topVec[2])};
				int[] frontRounded = {(int) Math.round(frontVec[0]), (int) Math.round(frontVec[1]), (int) Math.round(frontVec[2])};
				System.out.printf("(%d, %d) ", getSideForm(topRounded), getSideForm(frontRounded));
			}
			System.out.println("");
		}
	}
