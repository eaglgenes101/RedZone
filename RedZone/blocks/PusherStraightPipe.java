 ...
 * block or item will be destroyed if this transfer fails.
 * 
 * @author eaglgenes101
 * @see Pipe
 * @see StraightPipe
...
public class PusherStraightPipe extends StraightPipe implements PoweredComponent
{

	public PusherStraightPipe(String n)
	{
		super(n);
		...
		rightname = "RedZone_res/res/blocks/pusher_pipe_edge.png";
		frontname = "RedZone_res/res/blocks/pusher_pipe_edge.png";
		backname = "RedZone_res/res/blocks/pusher_pipe_edge.png";
	}
	...
	public void tickMeFast(World w, int d, int x, int y, int z)
	{
		...
		{ // where did our entity go???
			if (w.isServer)
			{
				...
				Entity eb = w.createEntityByName("RedZone:EntityPusherStraightPipe", d, x + 0.5, y + 0.5, z + 0.5);
				if (eb != null)
				{
					eb.init();
	...
	public void onBlockPlaced(World w, int dimension, int x, int y, int z)
	{
		if (w.isServer)
		{
			...
			Entity eb = w.createEntityByName("RedZone:EntityPusherStraightPipe", dimension, x + 0.5, y + 0.5, z + 0.5);
			if (eb != null)
			{
				eb.init();
				w.spawnEntityInWorld(eb);
