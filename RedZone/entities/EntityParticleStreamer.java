...
import blocks.ParticleStreamer;
 ...
 * 
 * 
 * Particle Streamer Entity. 
 * 
/*/

public class EntityParticleStreamer extends Entity
{

	public EntityParticleStreamer(World w)
	{
		super(w);
		uniquename = "RedZone:EntityParticleStreamer";
	...
	public void update(float deltaT)
	{
		...
		if (myBlockID != RedZoneBlocks.PARTICLE_STREAMER.blockID)
		{
			System.out.println("Bye!");
			this.deadflag = true;
			return;
		}
		else if (((ParticleStreamer) myBlock).getStatus(world, dimension, (int) posx, (int) posy, (int) posz))
		{
			if (FastBlockTicker.cycle % 2 != getVarInt(21))
			{
				rightclick(this.world);
	...
	public void rightclick(World world)
	{
		...
		Particle p = world.createParticleByName("DangerZone:ParticleHurt", dimension, posx, posy, posz);
		if (p != null)
		{
			p.motionx = (float) (5*getRelativeForward[0]);
			p.motiony = (float) (5*getRelativeForward[1]);
			p.motionz = (float) (5*getRelativeForward[2]);
			System.out.printf("[%f, %f, %f]\n", p.motionx, p.motiony, p.motionz);
			world.spawnParticleInWorld(p);
		}
		else
